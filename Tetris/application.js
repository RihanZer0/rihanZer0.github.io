...
Tetromino.prototype = {
	...
	// fn(x, y), принимает координаты каждого блока, возвращает true для break
    ...
    all: function (fn) { 
	// fn(x, y), принимает координаты каждого блока
        this.first(this.x, this.y, this.orientation, fn, false); // без break
...
const game = {
    ...
    blocks: [],
    queue: [],
    duration: 0,	// текущей игры
    score: 0,
    rows: 0,	// убранные ряды
    delay: 0,	// перед следующим смещении вниз на 1 ряд
    current: null,
    next: null,
    ...
    setState: function (aState) {
        ...
        window.focus();
        if (disabledControls) { // для FireFox
	...
	// предпологаемое движение блокируется каким-то препятствием
    ...
    keydownBody: function (event) {
        ...
        if (this.states.current === this.states.playing) {
            switch (event.keyCode) {
                ...
                case this.effectiveSettings.key.dropDown.keyCode:
                    // используется this.repeatedKeyDropDown потому что не все мобильные браузеры корректно поддерживают event.repeat
        ...
        if (handled)
            event.preventDefault(); // предотвращает скроллинг страницы стрелками
    }, //keydownBody

    repeatedKeyDropDown: false, //используется потому что не все мобильные браузеры корректно поддерживают event.repeat
...
const rendering = {
    ...
    draw: function () {
        ...
        const drawUpcoming = function (context) {
            ...
            const padding = (layoutMetrics.upcomingPreviewSize - game.next.shape.size) / 2; // попытка центрирования показа следующего тетромино
            context.save();
            finerLines(context);
            ...
            drawTetrominoAt(context, game.next, { x: padding, y: 1 });
            context.restore();
        ...
        const drawBoard = function (context) {
            ...
            context.strokeRect(0, 0, context.canvas.width - 1, context.canvas.height - 1); // граница доски
            context.restore();
        ...
        const drawState = function () {
            ...
            setText(elements.statusVerb, game.states.current === game.states.paused ? UiTexts.statusWordSet.continue : game.states.current === game.states.playing ? UiTexts.statusWordSet.pause : UiTexts.statusWordSet.start);
			setVisibility(elements.pausedText, game.states.current === game.states.paused);
			setVisibility(elements.OverText, game.states.current === game.states.cancelled);
